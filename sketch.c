 ...
 */
void mm_sketch(void *km, const char *str, int len, int w1, int k, uint32_t rid, int is_hpc, mm128_v *p)
{
	int w = 1;
	int skipp = 0;
	...
	for (i = l = buf_pos = min_pos = 0; i < len; ++i) {
		...
		// this is to sample every w1 length of the k-mers
		if (w1 > 1){
			if (l >= k){
				skipp++;
				if (skipp > 1){
		...
		if (info.x <= min.x) { // a new minimum; then write the old min
			...
			for (j = buf_pos + 1, min.x = UINT64_MAX; j < w; ++j) // the two loops are necessary when there are identical k-mers
				if (min.x >= buf[j].x) { // >= is important s.t. min is always the closest k-mer
					min = buf[j];
					min_pos = j;
				}
			for (j = 0; j <= buf_pos; ++j)
