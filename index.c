...
void mm_idx_stat(const mm_idx_t *mi)
{
	...
	//fprintf(stderr, "[INFO] number of references: %d\n", mi->n_seq);
	...
	fprintf(stderr, "[INFO] number of references: %d, total bases: %ld\n", mi->n_seq, (long)len);
...
mm_idx_t *mm_idx_gen(mm_bseq_file_t *fp, int w, int k, int b, int flag, int mini_batch_size, int n_threads, uint64_t batch_size)
{
	...
	if (mm_verbose >= 3)
		fprintf(stderr, "\n[INFO] hash table created\n");
		//fprintf(stderr, "[Info:%s::%.3f*%.2f] collected minimizers\n", __func__, realtime() - mm_realtime0, cputime() / (realtime() - mm_realtime0));

	mm_idx_post(pl.mi, n_threads);
	if (mm_verbose >= 3)
		fprintf(stderr, "[INFO] hash table sorted\n");
		//fprintf(stderr, "[Info:%s::%.3f*%.2f] sorted minimizers\n", __func__, realtime() - mm_realtime0, cputime() / (realtime() - mm_realtime0));
